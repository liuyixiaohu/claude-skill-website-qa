...
# ── Load data ────────────────────────────────────────────────
with open(os.path.join(OUT, "hybrid_automated_results.json"), "r") as f:
    data = json.load(f)
...
def slice_image(img_path, display_w_inches, max_h_inches=MAX_PAGE_H):
    ...
    with Image.open(img_path) as img:
        while y < h:
            ...
            seg_path = img_path.replace(".png", f"_s{i}.png")
...
def group_checks_by_category(checks):
    """Group checks into an OrderedDict keyed by category name."""
    groups = OrderedDict()
    for c in checks:
...
# Map category names to the part labels used in summary
CATEGORY_PART_MAP = {
    "Page Load & Layout": "part1",
    "Typography & Colors": "part2",
    "Images & Media": "part3",
    "Responsiveness": "part4",
    "Interactions": "part5",
    "Console & Network": "part6",
}
...
# Determine verdict (use stored verdict if available, otherwise compute)
...
# Summary table (Category x Passed x Failed x Total)
...
if summary:
    ...
    for part_key in sorted(summary.keys()):
        ...
        cell_text(row[3], str(part["total"]), size=Pt(9), align=WD_ALIGN_PARAGRAPH.CENTER)
        grand_passed += part["passed"]
        grand_failed += part["failed"]
    ...
    cell_text(
        ...
        size=Pt(9),
        bold=True,
        color=RED if grand_failed > 0 else GREEN,
        align=WD_ALIGN_PARAGRAPH.CENTER,
    )
    cell_text(
        row[3],
...
# Part number mapping for sub-headings
part_number = 1
for cat_name, cat_checks in grouped.items():
    ...
    hdr[0].width = Inches(0.5)
    hdr[1].width = Inches(2.2)
    hdr[2].width = Inches(4.8)
    for i, txt in enumerate(["Status", "Check", "Details"]):
...
if css_data:
    ...
    for i, vp in enumerate(viewports_to_show):
        ...
        cell_text(
            hdr[i + 1],
            viewport_labels.get(vp, vp.replace("_", " ")),
            size=Pt(7),
            bold=True,
        )
...
desktop_css = css_data.get("MacBook_Air_13", [])
sec_bgs = []
...
if sec_bgs:
    ...
    hdr[3].width = Inches(1.0)
    hdr[4].width = Inches(1.0)
    ...
    for bg in sec_bgs:
        ...
        bg_color = bg.get("backgroundColor", "?")
        bg_image = bg.get("backgroundImage", "?")
        height_val = bg.get("height", "?")
        padding_val = bg.get("padding", "?")

        cell_text(row[0], heading_text, size=Pt(7))
        cell_text(row[1], bg_color, size=Pt(7))
        cell_text(row[2], bg_image, size=Pt(7))
        cell_text(row[3], height_val, size=Pt(7))
        cell_text(row[4], padding_val, size=Pt(7))
else:
...
if mcp_data:
    ...
    # 6b. Responsive finding (tablet bug note)
...
# ── 7. Physical AI — Section Screenshots ─────────────────────

doc.add_heading("Physical AI \u2014 Section Screenshots", level=1)
section_screenshots = data.get("section_screenshots", {}).get("physical-ai", {})

# 7a. Desktop (full width)
SHOW_DESKTOP = "MacBook_Air_13"
desktop_secs = section_screenshots.get(SHOW_DESKTOP, [])

if desktop_secs:
    doc.add_heading('Desktop \u2014 MacBook Air 13" (1440\u00d7900)', level=2)
...
SHOW_TABLET = "iPad_Air"
SHOW_MOBILE = "iPhone_15"
tablet_secs = section_screenshots.get(SHOW_TABLET, [])
mobile_secs = section_screenshots.get(SHOW_MOBILE, [])
...
# Desktop contact modal (full width)
...
# Mobile interactions side by side (Contact modal + Hamburger)
mobile_interactions = [
